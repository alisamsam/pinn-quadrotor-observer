"""Plot all 12 state estimates vs truth (lambda=1 model)."""
import sys, os
...
NOISE_STD = 0.02
TRAJ_IDX = 0
MODEL_FILE = "models/pinn_lam1.0.pth"     # the balanced (best) observer

# 12 state names + units, in order
STATE_INFO = [
    ("x", "m"), ("y", "m"), ("z", "m"),
...
# ----- 4x3 grid of subplots -----
fig, axes = plt.subplots(4, 3, figsize=(15, 12))
axes = axes.flatten()
for i, (name, unit) in enumerate(STATE_INFO):
    ax = axes[i]
    ax.plot(T, x_true[:, i], 'g-', linewidth=2, label="True")
    ax.plot(T, y_noisy[:, i], 'k.', markersize=1.5, alpha=0.25, label="Noisy")
    ax.plot(T, x_est[:, i], 'r--', linewidth=1.3, label="Estimated")
    ax.set_title(f"{name}  [{unit}]", fontsize=11)
    ax.grid(True, alpha=0.3)
    if i == 0:
        ax.legend(fontsize=8, loc="best")

fig.suptitle("PINN observer (lambda=1): all 12 states, trajectory 0", fontsize=15)
fig.tight_layout(rect=[0, 0, 1, 0.98])

os.makedirs("docs", exist_ok=True)
plt.savefig("docs/eval_all_states.png", dpi=110, bbox_inches="tight")
print("Saved -> docs/eval_all_states.png")