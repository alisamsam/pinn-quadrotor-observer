"""B.8: evaluate observer on rich controlled flight, all 12 states."""
import sys, os
...
m = PINNObserver(); m.load_state_dict(torch.load("models/pinn_controlled.pth")); m.eval()
with torch.no_grad():
...
for i,(name,unit) in enumerate(STATE_INFO):
    ...
    ax.plot(T, y_noisy[:,i], 'k.', ms=1.5, alpha=0.2, label="Noisy")
    ax.plot(T, x_est[:,i], 'b-', lw=1.2, label="Est")
    ax.set_title(f"{name} [{unit}]", fontsize=11); ax.grid(alpha=0.3)
    if i==0: ax.legend(fontsize=8)
fig.suptitle("B.8: observer on rich controlled flight, all 12 states", fontsize=15)
fig.tight_layout(rect=[0,0,1,0.98])
os.makedirs("docs", exist_ok=True)
plt.savefig("docs/eval_controlled.png", dpi=110, bbox_inches="tight")
print("Saved -> docs/eval_controlled.png")