# -*- coding: utf-8 -*-
# Plot the CIRCLE figures in HD from the exported CSV.
# Reads:  paper_figures/fig_circle_observer_states.csv
# Writes (vector PDF + 300-dpi PNG) into paper_figures/:
#   circle_observer_3d.pdf/.png       (true vs estimated 3-D trajectory)
#   circle_observer_12states.pdf/.png (true vs estimated, all 12 states)
import numpy as np
import matplotlib
matplotlib.use("Agg")
...
d = np.genfromtxt("paper_figures/fig_circle_observer_states.csv", delimiter=",", names=True)
t = d["t"]
...
ax.set_title("Circle: PINN observer, true vs estimated (3-D)")
ax.legend(); ax.view_init(elev=18, azim=-60)
save(fig, "paper_figures/circle_observer_3d")
...
fig.suptitle("Circle: PINN observer on an unseen flight -- true vs estimated (all 12 states)",
             fontsize=13)
fig.tight_layout(rect=[0, 0, 1, 0.98])
save(fig, "paper_figures/circle_observer_12states")
print("DONE.")
