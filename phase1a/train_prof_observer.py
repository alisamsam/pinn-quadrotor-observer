...
SEED = 0
SAVE_PATH = "phase1a/pinn_prof_4x100.pth"

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
print("Device:", device, flush=True)
print(f"Training {LAYERS}x{HIDDEN} on PROFESSOR's data, weights ({W0},{WODE},{WY})", flush=True)

train, test = load_phase4_data("datasets/spiral_prof_dataset.npz")
...
print(f"(compare: YOUR controller observer hidden RMSE 0.0331; band +/-0.009; his trajectory gentler)", flush=True)