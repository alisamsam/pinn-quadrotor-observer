...
sets = ["mass_unaware_-20","mass_unaware_-10","mass_unaware_0",
        "mass_unaware_+10","mass_unaware_+20"]
rows = []
for tag in sets:
    ...
    print(f"{tag:>20} (mass {dev*100:+.0f}%): meas {mr:.4f} | hidden {hr:.4f}", flush=True)

os.makedirs("docs", exist_ok=True)
with open("docs/mismatch_mass_unaware.csv","w",newline="") as fp:
    w = csv.writer(fp); w.writerow(["set","mass_deviation","test_meas_RMSE","test_hidden_RMSE"]); w.writerows(rows)
print("\nSaved docs/mismatch_mass_unaware.csv", flush=True)