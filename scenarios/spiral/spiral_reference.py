...
OMEGA = 1.0       # angular rate (raw Table 5 form; ω²R = 10 m/s² ≈ g, aggressive)
...
def spiral_reference(t):
    ...
    x_d = R * np.sin(w*t)
    y_d = R * np.cos(w*t)
    z_d = CLIMB * t                 # CHANGED: climbs instead of constant
    # velocity
    xdot_d =  R * w * np.cos(w*t)
    ydot_d = -R * w * np.sin(w*t)
    zdot_d = CLIMB                  # CHANGED: constant climb rate (not 0)
    # acceleration
    xddot_d = -R * w**2 * np.sin(w*t)
    yddot_d = -R * w**2 * np.cos(w*t)
    zddot_d = 0.0                   # constant velocity climb → zero z-accel
    return (np.array([x_d, y_d, z_d]),
            np.array([xdot_d, ydot_d, zdot_d]),
            np.array([xddot_d, yddot_d, zddot_d]))
def yaw_reference(t):
    psi_d = PSI_D                   # CHANGED: constant yaw (not sinusoidal)
    psi_d_dot = 0.0                 # CHANGED: constant yaw → zero rate
