"""controller_v2 on the spiral: (1) standalone 3D figure, (2) all-12-states grid."""
import sys, os
...
t, Y = sol.t, sol.y
P = np.array([spiral_reference(tt) for tt in t], dtype=object)
pos_d = np.array([spiral_reference(tt)[0] for tt in t])
vel_d = np.array([spiral_reference(tt)[1] for tt in t])
...
for k in range(len(t)):
    x = Y[:, k]; p_d, v_d, a_d = spiral_reference(t[k])
...
ax.set_title('Spiral tracking (controller_v2): desired vs actual')
ax.legend(); ax.view_init(elev=18, azim=-60)
out3d = os.path.join(PROJ, "docs", "spiral_3d_v2.png")
plt.tight_layout(); plt.savefig(out3d, dpi=120, bbox_inches="tight"); plt.close(fig)
print("saved ->", out3d)


# ---------- Figure 2: all 12 states (desired overlaid where a reference exists) ----------
...
for i, ax in enumerate(axes.flat):
    ...
    ax.set_title(f'{labels[i]} ({units[i]})', fontsize=10)
    ax.grid(alpha=0.3); ax.set_xlabel('t (s)', fontsize=8)
    ax.legend(fontsize=7, loc='upper right')
fig.suptitle('All 12 states — spiral with controller_v2 (R=10, omega=0.6). '
             'p,q,r have no reference (actual only).', fontsize=12)
out12 = os.path.join(PROJ, "docs", "spiral_12states_v2.png")
