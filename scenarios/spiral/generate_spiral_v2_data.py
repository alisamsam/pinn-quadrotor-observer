"""Generate observer dataset from IMPROVED controller_v2 spiral flights.
Same 50-flight initial-condition scatter as generate_spiral_data.py (so the two
datasets are directly comparable); only the controller differs. New output file
datasets/spiral_v2_dataset.npz — the old spiral_dataset.npz is left untouched.
"""
import sys, os
...
def ctrl(t, x):
    return full_control_v2(t, x, spiral_reference, yaw_reference)
...
if __name__ == "__main__":
    ...
    np.savez("datasets/spiral_v2_dataset.npz", T=T, X=X, U=U)
    print("saved -> datasets/spiral_v2_dataset.npz")
