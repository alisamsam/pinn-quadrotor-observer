"""Plot U1-U4 control inputs over the circle flight (validation vs Singha)."""
import sys, os
...
# simulate one clean flight (Table 5 start: 0,0,4)
x0 = np.zeros(12)
x0[0], x0[1], x0[2] = 0.0, 0.0, 4.0
...
U = np.array([full_control_circle(t_eval[k], sol.y[:,k]) for k in range(len(t_eval))])
...
ax[0,0].legend()
fig.suptitle("Circle trajectory — control inputs U1–U4", fontsize=14)
fig.tight_layout()
os.makedirs("docs", exist_ok=True)
plt.savefig("docs/circle_controls.png", dpi=120, bbox_inches="tight")
print("Saved -> docs/circle_controls.png")
