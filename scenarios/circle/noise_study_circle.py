...
sys.path.insert(0, os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), "phase1a"))
...
# Noise-robustness study for the CIRCLE observer (same protocol as the spiral study).
# Gaussian sensor noise is added to the MEASURED outputs used in training; evaluation
# is always vs the TRUE states of the ten unseen test flights.
# Circle best config: 4x100, weights (0.5, 0.5, 1.0), circle_v3 dataset.
# Run from repo root:
#   module load pytorch/2.0.0/gpu
#   nohup python3 scenarios/circle/noise_study_circle.py >> noise_circle_log.txt 2>&1 &
...
POS_IDX_IN_Y = [0, 1, 2]     # measured y = [x, y, z, phi, theta, psi]
ATT_IDX_IN_Y = [3, 4, 5]

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
print("Device:", device, flush=True)
print(f"Circle noise study on {DATASET}, {LAYERS}x{HIDDEN}, weights ({W0},{WODE},{WY})", flush=True)
...
with open("docs/noise_study_circle.csv", "w", newline="") as fp:
    w = csv.writer(fp)
    w.writerow(["case","sigma_pos_m","sigma_att_rad","test_meas_RMSE","test_hidden_RMSE","train_time_s"])
    w.writerows(rows)

print("\n================ CIRCLE NOISE SUMMARY ================", flush=True)
for r in rows:
    print(f"  {r[0]:>9}: meas {r[3]} | hidden {r[4]}", flush=True)
print("Saved docs/noise_study_circle.csv", flush=True)
