# -*- coding: utf-8 -*-
# CIRCLE design selection on the 10-flight metric (same as the figure-8 study):
...
# Every RMSE is POOLED over the 10 unseen test flights, so the sweep numbers
# are consistent with the final accuracy table.
# Saves docs/grid_study_circle_10flight.csv and docs/weight_study_circle_10flight.csv
#
# Run from repo root (cluster):
#   module load pytorch/2.0.0/gpu
#   nohup python3 scenarios/circle/design_study_circle_10f.py >> circle_design_10f_log.txt 2>&1 &
# To shorten: edit LAYERS below (e.g. drop 12).
import sys, os, time
ROOT = os.getcwd()
...
print('Device:', device, '| circle design study (10-flight)', flush=True)
...
for L in LAYERS:
    for W in WIDTHS:
        ...
        print('  %2d x %3d | params %7d | meas %.4f | hidden %.4f | %.0fs' % (L, W, pp, mm, hh, dt), flush=True)
with open('docs/grid_study_circle_10flight.csv', 'w') as f:
...
with open('docs/weight_study_circle_10flight.csv', 'w') as f:
...
print('  BEST weights: C%d (%.1f,%.1f,%.1f) hidden %.4f' % (bestw[0], bestw[1], bestw[2], bestw[3], bestw[6]), flush=True)

# --- final per-state accuracy AT the selected (architecture, weights) ---
...
print('\n== final per-state accuracy at %dx%d w=(%.1f,%.1f,%.1f) ==' % (bestL, bestW, fw0, fwode, fwy), flush=True)
torch.manual_seed(SEED)
fm = PINNObserverV4(hidden=bestW, n_hidden_layers=bestL).to(device)
fopt = torch.optim.Adam(fm.parameters(), lr=LR)
for ep in range(1, EPOCHS + 1):
    perm = torch.randperm(N, device=device)
    for s in range(0, N, BATCH):
        idx = perm[s:s + BATCH]; tb, x0b, yb, ub = T[idx], X0[idx], Y[idx], U[idx]
        fopt.zero_grad(); xh = fm(tb, x0b)
        ly = nn.functional.mse_loss(xh[:, MEAS_IDX], yb); res = residual(fm, tb, x0b, ub)
        lg = nn.functional.mse_loss(res, torch.zeros_like(res)); l0 = nn.functional.mse_loss(fm(T0, X0_0), Xtrue_0)
        (fwy * ly + fwode * lg + fw0 * l0).backward(); fopt.step()
    if ep == 1 or ep % LOG_EVERY == 0:
        print('    [final] ep %4d' % ep, flush=True)
fm.eval()
with torch.no_grad():
    est = fm(Tt, X0t).cpu().numpy()
ps = np.sqrt(((est - Xt_true) ** 2).mean(axis=0))
...
print('\nDONE. Saved grid + weight + per-state CSVs for circle (all 10-flight, same config).', flush=True)
