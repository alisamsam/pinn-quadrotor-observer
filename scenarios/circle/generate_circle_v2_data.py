"""Generate observer dataset from IMPROVED controller_v2 CIRCLE flights.
Same 50-flight initial-condition scatter as the spiral_v2 generator (so the datasets
are directly comparable); only the reference trajectory differs (circle instead of
spiral). Output: datasets/circle_v2_dataset.npz.
"""
import sys, os
...
def ctrl(t, x):
    return full_control_v2(t, x, circular_reference, yaw_reference)
...
if __name__ == "__main__":
    ...
    np.savez("datasets/circle_v2_dataset.npz", T=T, X=X, U=U)
    print("saved -> datasets/circle_v2_dataset.npz")
