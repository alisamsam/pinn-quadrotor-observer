# -*- coding: utf-8 -*-
# Train the FIGURE-8 observer at its best config (9 layers x 100 neurons, weights 1/1/1)
...
#   nohup python eval_figure8_best.py >> eval_figure8_best_log.txt 2>&1 &
import sys, os, time
ROOT = os.getcwd()
...
LAYERS, HIDDEN = 9, 100
W0, WODE, WY = 1.0, 1.0, 1.0          # best figure-8 weight case (Case 1, neutral)
SEED = 0
DATASET = 'datasets/figure8_v3_dataset.npz'
NSTEPS = 3000; FLIGHT = 0

device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
print('Device:', device, '| figure-8 best observer 9x100 w=(1,1,1)', flush=True)
...
rmse=np.sqrt(((est-true)**2).mean(axis=0)); HID=[i for i in range(12) if i not in MEAS_IDX]
...
out='docs/eval_figure8_best_data.npz'
