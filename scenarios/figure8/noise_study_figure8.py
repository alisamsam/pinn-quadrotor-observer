...
# Noise-robustness study for the FIGURE-8 observer (same protocol as spiral/circle).
...
# Figure-8 best config: 9x100, weights (1,1,1), figure8_v3 dataset.
# Run from repo root:
#   module load pytorch/2.0.0/gpu
#   nohup python3 scenarios/figure8/noise_study_figure8.py >> noise_figure8_log.txt 2>&1 &
...
print(f"Figure-8 noise study on {DATASET}, {LAYERS}x{HIDDEN}, weights ({W0},{WODE},{WY})", flush=True)
...
with open("docs/noise_study_figure8.csv", "w", newline="") as fp:
    w = csv.writer(fp)
    w.writerow(["case","sigma_pos_m","sigma_att_rad","test_meas_RMSE","test_hidden_RMSE","train_time_s"])
    w.writerows(rows)

print("\n================ FIGURE-8 NOISE SUMMARY ================", flush=True)
for r in rows:
    print(f"  {r[0]:>9}: meas {r[3]} | hidden {r[4]}", flush=True)
print("Saved docs/noise_study_figure8.csv", flush=True)
