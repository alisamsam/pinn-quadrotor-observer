"""Evaluate the PINN observer on Figure8-tracking flight, all 12 states."""
import sys, os
...
data = np.load("datasets/figure8_dataset.npz")
T = data["T"]; X = data["X"]
x_true = X[TRAJ_IDX]
...
m.load_state_dict(torch.load("models/pinn_figure8.pth")); m.eval()
with torch.no_grad():
...
fig.suptitle("Figure8 scenario: observer on all 12 states", fontsize=15)
fig.tight_layout(rect=[0,0,1,0.98])
os.makedirs("docs", exist_ok=True)
plt.savefig("docs/eval_figure8.png", dpi=110, bbox_inches="tight")
print("Saved -> docs/eval_figure8.png")