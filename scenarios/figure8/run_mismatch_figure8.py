...
(nominal-trained) is evaluated vs the true states. Writes docs/figure8_mismatch_summary.csv.

FULLY SELF-CONTAINED: the controller (controller_v3), the figure-8 reference, the
perturbed Singha dynamics and a fixed-step RK4 (dt = 0.01 s) are all inlined, so the
ONLY repo imports are PINNObserverV4 / quadrotor_dynamics_torch / data_phase4. No scipy,
no matplotlib, no core.controller_v3 / scenarios.* needed on the cluster.

Figure-8 observer config: 9x100, loss weights (1.0, 1.0, 1.0), figure8_v3 dataset.
If phase1a/pinn_figure8_9x100.pth is absent it is trained once and cached, then frozen.
...
# ===== figure-8 reference (scenarios/figure8/figure8_reference.py) =====
PSI_CONST, OMEGA = 0.7853, 0.3
def figure8_reference(t):
    w = OMEGA
    return (np.array([10*np.sin(w*t), 10*np.cos(w*t)*np.sin(w*t), 1.5 + np.cos(w*t)]),
            np.array([10*w*np.cos(w*t), 10*w*np.cos(2*w*t), -w*np.sin(w*t)]),
            np.array([-10*w**2*np.sin(w*t), -20*w**2*np.sin(2*w*t), -w**2*np.cos(w*t)]))
def yaw_reference(t):
    return PSI_CONST, 0.0
...
def rk4_rollout(P, x0, t_eval):
    def deriv(tt, xx):
        return quadrotor_dynamics(xx, full_control_v3(tt, xx, figure8_reference, yaw_reference), P)
...
CKPT    = "phase1a/pinn_figure8_9x100.pth"
LAYERS, HIDDEN = 9, 100
...
with open("docs/figure8_mismatch_summary.csv", "w", newline="") as fp:
...
print("saved -> docs/figure8_mismatch_summary.csv", flush=True)
